...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  $sql = "SELECT * FROM sign_up WHERE Email='$email' AND Password='$password' AND Role='admin'";
  $result = $conn->query($sql);

  if ($result && $result->num_rows == 1) {
    $user = $result->fetch_assoc(); // ✅ fetch user row
    $_SESSION['email'] = $user['Email'];
    $_SESSION['Role'] = $user['Role'];

    echo "<script>alert('Successfully Logged In!');window.location.href='admindashboard.php';</script>";
    exit();
  } else {
